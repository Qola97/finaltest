...
def memo_for1():
    global a
    if request.method == 'POST':
        if request.form['memo1']!='':
            a = request.form['memo1']
            return redirect(url_for('memo1.html'))
        else:
            a = request.form['num']
            if a =='':
                a = None
            return render_template(url_for('memo1.html'))
@app.route('/')
def memo():
    return render_template(url_for('memo.html'))
...
def save_memo():
    ...
    if a is not None:
        if but == 'save' :
            g.db.execute('''insert into message ( memo1) values (?)''', a)
            g.db.commit()
            flash('Your memo was recorded')
        else:
            a = None
    return render_template('memo1.html', memo1 = a, but=but)
